...
properties, ``tag``, ```sourceRepository```, and ```targetRepository```.
...
The target repository is a required configuration property specified via ```-DtargetRepository``` or within the 
plugin 
configuration in the pom file.

#####Move Usage Example
e.g. ```mvn nexus-staging:move -Dtag=build-123 -DsourceRepository=maven-dev -DtargetRepository=maven-qa``` 
